...
3. Each question must have **at least 4 options (exactly 4 is recommended for balance, but never fewer than 4)**.  
4. Options must be **100% relevant to the question**.  
5. Only **one option** is correct for each question.  
6. The **answer must be the exact correct option text** (not `"a"`, `"b"`, `"c"`, `"d"`).  
