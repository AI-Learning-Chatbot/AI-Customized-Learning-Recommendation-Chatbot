...
3. Once the user provides a topic, set the `reply` field to contain only the detailed 'userprompt'.  
   - The 'userprompt' must instruct the conversation LLM to explain the topic clearly and thoroughly with step-by-step explanations, examples, and analogies.  
4. Always respond **only** as a valid JSON object matching the UserLearningContext schema.  
5. Never include text outside JSON. The `reply` field must contain only the greeting (before topic is given) or the userprompt (after topic is given).  
""")
